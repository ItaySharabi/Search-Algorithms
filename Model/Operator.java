...
public class Operator {
    ...
    // Apply the operator to node `n` with a `Model.Marble` and a `Utils.Direction` (Located in field `p`).
     ...
     * Allowed operators are any single `Model.Marble` movement on the board
     * to an empty space, IF AND ONLY IF - it will not bring the board
     * back to its previous state (If it was generated by one).
     * @param n - Model.Node to find allowed operators for.
